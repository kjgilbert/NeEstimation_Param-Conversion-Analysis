### READ NEMO FSTAT OUTPUT FILES INTO USEFUL FORMAT


read.nemo <- function (currentdata, ancientdata, ninds, nsub, maxall=256, nsamps=2, gensbtwn="4.00", mlne.gens="4", maxNe=1000, na.s = c("0", "00", "000", "0000", "00000", "000000", "NA")) 
{
...
#NEED TO MAKE SURE IN THE END WE'RE SAMPLING THE SAME LOCI AT BOTH TIME POINTS; this is done below the following code  
# keep all loci even if there are rare alleles, and let programs deal with rare alleles however they will
#    		if(length(table(dat[,k]))>1){
#    			maj.a.f <- max(table(dat[,k])[]) / sum(table(dat[,k])[])
#    			if(maj.a.f>0.95){ remove.loci <- c(remove.loci, k)} #if an allele at a locus is > 95%, remove that locus
#    			}
    	}
...
x2 <- scan(ancientdata, n = 4) #read first line of file, gives number of loci, alleles, etc
    nloc2 <- x2[2]-4  #get number of loci (-4 because nemo adds 4 extra rows where loci are)
    lnames2 <- scan(ancientdata, what = character(), skip = 1, nlines = nloc2)  #locus names
    lnames2 <- c("Pop", lnames2)  #add first column heading name to be "pop"
    dat2 <- scan(ancientdata, skip = nloc2 + 5, what = character(), na.strings = na.s)  #read genotype data
    	#adding what=character reads data as characters instead of numbers and keeps the leading zeroes
    dat2 <- data.frame(matrix(dat2, ncol = nloc2 + 5, byrow = TRUE))  #genotype data in matrix format now
    dat2 <- dat2[,-((length(dat2[1,])-3):length(dat2[1,]))]  #remove last 4 columns of nemo data
    names(dat2) <- lnames2  #add column names
    
    
#split genotypes into alleles of 3 digits
    for(i in 2:(nloc2+1)){  #go from first column of genotype data to the last column
    	if(i %% 10==0) print(i)	
    	ans2 <- matrix(NA, ncol=2, nrow=length(dat2[,i]))
		for(j in 1:length(dat2[,i])){
			test2 <- strsplit(as.character(dat2[,i]), "")[[j]]
			ans2[j,1] <- paste0(test2[c(1,4)], test2[c(2,5)], test2[c(3,6)])[1]
			ans2[j,2] <- paste0(test2[c(1,4)], test2[c(2,5)], test2[c(3,6)])[2]
			}  #end for loop going through individual genotype column, splitting genotypes
		dat2[,i] <- ans2
    	}  #end for loop going through genotype columns
...
#    		if(length(table(dat2[,k]))>1){
#    			maj.a.f2 <- max(table(dat2[,k])[]) / sum(table(dat2[,k])[])
#    			if(maj.a.f2>0.95){ remove.loci2 <- c(remove.loci2, k)} #if an allele at a locus is > 95%, remove that locus
#    			}
    	}
...
finalcurrentdat <- finalcurrentdat_presubset[who.to.sample,]
finalancientdat <- finalancientdat_presubset[who.to.sample,]
...
for(kk in 2:(number.of.loci+1)){
	...
	for(jj in 1:length(finalcurrentdat[,kk])){
...
#now make each row a character
	...
	writeLines(onesamp_input, paste("LongOutputs/ONeSAMPIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
##  ##  PATCH 1  ##  ##
NEcurrdat <- finalcurrentdat	
NEancdat <- finalancientdat	
...
#now make each row a character
	...
	writeLines(NeEstimator_input, paste("LongOutputs/NeEstIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
	

####################################################################################################
##   LDNe
# FSTAT format: first row: # pops, # loci, max # alleles, # digits per allele

#LDdat <- finalcurrentdat
...
#	if(xx %% 10==0) print(xx)
#	#go through each genotype column
#	for(zz in 1:length(LDdat[,1])){
#		LDdat[zz, xx] <- paste(LDdat[zz, xx], collapse="")
#		}
#		LDdat[,xx] <- as.matrix(LDdat[,xx][,1])
#	}
	
#make locus rows for first part of file
#LDrows <- paste(finalnames, sep="\n")

#now fix number of loci listed in the file to its actual number, because x[2] before is number of loci plus 4 for the 4 extra columns nemo adds
#	LDrow1 <- x
#   	LDrow1[2] <- number.of.loci
		 	
#now make each row a character
#	finalLDdat <- NULL
#	for(qq in 1:length(LDdat[,1])){
#		aa <- paste(LDdat[qq,], collapse=" ")
#		finalLDdat <- c(finalLDdat, aa)
#	}
	
#	ldne_input <- paste(c(paste(as.character(LDrow1), collapse=" "), LDrows, finalLDdat), sep="/n")

#	writeLines(ldne_input, paste("LongOutputs/LDNeIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
#now make each row a character
	...
	writeLines(estim_input, paste("LongOutputs/EstimIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
tmvp.currdat <- finalcurrentdat
tmvp.ancdat <- finalancientdat
...
tmvp_input <- paste(c(paste(number.of.loci), fulllist, sep="\n"))  #add number of loci at start of file
	...
	writeLines(tmvp_input, paste("LongOutputs/TMVPIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
mlne.currdat <- finalcurrentdat
mlne.ancdat <- finalancientdat
...
mlne_input <- paste(c(paste("0"), paste("0"), paste(maxNe), paste("2"), paste("0"), paste(number.of.loci), paste(allele.list, collapse=","), paste(nsamps), between, "", mlne.fulllist, "", paste("1"), sep="\n"))  #add 0, number of time points, and number of loci at start of file
	...
	writeLines(mlne_input, paste("LongOutputs/MLNeIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
cone.currdat <- finalcurrentdat
cone.ancdat <- finalancientdat
...
for(m in 2:(number.of.loci+1)){
	...
	writeLines(cone_input, paste("LongOutputs/CoNeIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
...
colony.currdat <- finalcurrentdat
...
colony_input <- paste(c(
	...
	writeLines(colony_input, paste("LongOutputs/ColonyIN_", tail(unlist(strsplit(currentdata, "/")),n=1), sep=""))
